...
def test_documented_cli_commands_run_in_dependency_aware_sequence(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    class SequenceFakeClient(FakeClient):
        created_key_id = "key_created_456"
        created_config_id = "cfg_created_456"
        optimize_job_id = "job_optimize_456"
        compile_job_id = "job_compile_456"
        compiled_function_id = "fn_compiled_456"
        extract_job_id = "job_extract_456"

        def create_api_key(self, payload: dict[str, object]) -> dict[str, object]:
            self.calls.append(("create_api_key", payload))
            return {
                "key_id": self.created_key_id,
                "name": str(payload.get("name", "New key")),
                "scope": str(payload.get("scope", "full")),
                "key_value": "sk_live_created",
            }

        def create_llm_config(self, payload: dict[str, object]) -> dict[str, object]:
            self.calls.append(("create_llm_config", payload))
            return {"config_id": self.created_config_id, "status": "created"}

        def compile_function(self, payload: dict[str, object]) -> dict[str, object]:
            self.calls.append(("compile_function", payload))
            return {"job_id": self.compile_job_id, "status": "queued"}

        def optimize_prompt(self, payload: dict[str, object]) -> dict[str, object]:
            self.calls.append(("optimize_prompt", payload))
            return {"job_id": self.optimize_job_id, "status": "queued"}

        def extract(self, payload: dict[str, object]) -> dict[str, object]:
            self.calls.append(("extract", payload))
            return {"job_id": self.extract_job_id, "status": "queued"}

        def get_function(self, function_id: str) -> dict[str, object]:
            self.calls.append(("get_function", {"function_id": function_id}))
            return {"function_id": function_id, "name": "Compiled function"}
        ...
        def list_extractions(self) -> list[dict[str, object]]:
            self.calls.append(("list_extractions", None))
            return [{"job_id": self.extract_job_id, "type": "extract", "status": "completed"}]

    def run_command(args: list[str]) -> tuple[int, object, str]:
        command_name = args[0]
        ...
        return exit_code, _parse_cli_json_output(captured.out), captured.err

    _write_json(tmp_path / "org-update.json", {"name": "Renamed org"})
    _write_json(tmp_path / "api-key-create.json", {"name": "CLI key", "scope": "read_only"})
    _write_json(
    ...
    _write_json(tmp_path / "config-update.json", {"name": "Renamed config"})
    _write_json(
    ...
    _write_json(tmp_path / "function-update.json", {"name": "Renamed function"})
    _write_json(
    ...
    fake_client = SequenceFakeClient(api_key="sk_test")
    fake_client.set_job_sequence(
        fake_client.optimize_job_id,
        [
            {"job_id": fake_client.optimize_job_id, "status": "running", "progress": 40},
            {
                "job_id": fake_client.optimize_job_id,
                "status": "completed",
                "progress": 100,
                "result": {"optimized_prompts": ["Better prompt", "Fallback prompt"]},
            },
        ],
    )
    fake_client.set_job_sequence(
        fake_client.compile_job_id,
        [
            {"job_id": fake_client.compile_job_id, "status": "running", "progress": 55, "type": "compile"},
            {
                "job_id": fake_client.compile_job_id,
                "status": "completed",
                "progress": 100,
                "type": "compile",
                "result": {"function_id": fake_client.compiled_function_id},
            },
        ],
    )
    fake_client.set_job_sequence(
        fake_client.extract_job_id,
        [
            {"job_id": fake_client.extract_job_id, "status": "running", "progress": 70, "type": "extract"},
            {
                "job_id": fake_client.extract_job_id,
                "status": "completed",
                "progress": 100,
                "type": "extract",
                "result": {"field": "value"},
    ...
    exit_code, health_result, _ = run_command(["health"])
    assert exit_code == 0
    assert health_result == {"status": "ok", "version": "v1"}

    exit_code, models_result, _ = run_command(["models"])
    assert exit_code == 0
    assert models_result == {"models": ["google/gemini-2.5-flash-lite"]}

    exit_code, org_result, _ = run_command(["org", "--api-key", "sk_test"])
    assert exit_code == 0
    assert org_result == {"org_id": "org_123", "name": "Acme"}

    exit_code, org_update_result, _ = run_command(
        ["org-update", "--api-key", "sk_test", "--payload-file", ".\\org-update.json"]
    )
    assert exit_code == 0
    assert org_update_result == {"org_id": "org_123", "name": "Renamed org"}

    exit_code, api_keys_result, _ = run_command(["api-keys", "--api-key", "sk_test"])
    assert exit_code == 0
    assert api_keys_result == [{"key_id": "key_123", "name": "Primary", "scope": "full", "revoked": False}]

    exit_code, api_key_create_result, _ = run_command(
        ["api-key-create", "--api-key", "sk_test", "--payload-file", ".\\api-key-create.json"]
    )
    assert exit_code == 0
    assert api_key_create_result == {
        "key_id": fake_client.created_key_id,
        "name": "CLI key",
        "scope": "read_only",
        "key_value": "sk_live_created",
    }
    created_key_id = str(api_key_create_result["key_id"])

    exit_code, configs_result, _ = run_command(["configs", "--api-key", "sk_test"])
    assert exit_code == 0
    assert configs_result == [{"config_id": "cfg_123", "name": "Primary config"}]

    exit_code, config_result, _ = run_command(["config", "--api-key", "sk_test", "--payload-file", ".\\config-input.json"])
    assert exit_code == 0
    assert config_result == {"config_id": fake_client.created_config_id, "status": "created"}
    created_config_id = str(config_result["config_id"])

    exit_code, config_update_result, _ = run_command(
        [
            "config-update",
            "--api-key",
            "sk_test",
    ...
    assert exit_code == 0
    assert config_update_result == {"config_id": created_config_id, "name": "Renamed config"}

    exit_code, functions_result, _ = run_command(
        ["functions", "--api-key", "sk_test", "--name", "Invoice", "--tag", "billing"]
    )
    assert exit_code == 0
    assert functions_result == [{"function_id": "fn_123", "name": "Invoice", "tags": ["billing"]}]

    exit_code, optimize_result, _ = run_command(
        [
            "optimize",
            "--api-key",
            "sk_test",
    ...
    assert altered_compile_payload["compile_options"] == {
        "optimized_prompts": ["Better prompt", "Fallback prompt"]
    }

    exit_code, compile_result, _ = run_command(
        [
            "compile",
            "--api-key",
            "sk_test",
    ...
    compiled_function_id = str(compile_result["result"]["function_id"])

    exit_code, function_result, _ = run_command(
        ["function", "--api-key", "sk_test", "--function", compiled_function_id]
    )
    assert exit_code == 0
    assert function_result == {"function_id": compiled_function_id, "name": "Compiled function"}

    exit_code, function_update_result, _ = run_command(
        [
            "function-update",
            "--api-key",
            "sk_test",
    ...
    assert exit_code == 0
    assert function_update_result == {"function_id": compiled_function_id, "name": "Renamed function"}

    exit_code, extract_result, _ = run_command(
        [
            "extract",
            "--api-key",
            "sk_test",
    ...
    exit_code, extractions_result, _ = run_command(["extractions", "--api-key", "sk_test"])
    assert exit_code == 0
    assert extractions_result == [{"job_id": extract_job_id, "type": "extract", "status": "completed"}]

    exit_code, job_result, _ = run_command(["job", "--api-key", "sk_test", "--job", extract_job_id])
    assert exit_code == 0
    assert job_result == {
        "job_id": fake_client.extract_job_id,
        "status": "completed",
    ...
    exit_code, poll_result, _ = run_command(
        ["poll", "--api-key", "sk_test", "--job", extract_job_id, "--timeout", "600"]
    )
    assert exit_code == 0
    assert poll_result == {
        "job_id": fake_client.extract_job_id,
    ...
    exit_code, usage_result, _ = run_command(
        ["usage", "--api-key", "sk_test", "--from", "2026-04-01", "--to", "2026-04-15"]
    )
    assert exit_code == 0
    assert usage_result == {
        "org_id": "org_123",
    ...
    exit_code, state_result, _ = run_command(["state", "latest"])
    assert exit_code == 0
    assert state_result == {
        "state_file": "general",
        "config_id": created_config_id,
        "optimize_job_id": fake_client.optimize_job_id,
        "compile_job_id": fake_client.compile_job_id,
        "function_id": compiled_function_id,
        "extract_job_id": extract_job_id,
    }

    exit_code, api_key_revoke_result, _ = run_command(
        ["api-key-revoke", "--api-key", "sk_test", "--key", created_key_id]
    )
    assert exit_code == 0
    assert api_key_revoke_result is None

    exit_code, function_delete_result, _ = run_command(
        ["function-delete", "--api-key", "sk_test", "--function", compiled_function_id]
    )
    assert exit_code == 0
    assert function_delete_result is None

    exit_code, config_delete_result, _ = run_command(
        ["config-delete", "--api-key", "sk_test", "--config", created_config_id]
    )
    assert exit_code == 0
    assert config_delete_result is None

    assert ("revoke_api_key", {"key_id": created_key_id}) in fake_client.calls
    assert (
        "compile_function",
        {
            "name": "Example",
            ...
            "json_structure": {"field": "string"},
            "training_data": [],
            "compile_options": {"optimized_prompts": ["Better prompt", "Fallback prompt"]},
            "config_id": created_config_id,
        },
